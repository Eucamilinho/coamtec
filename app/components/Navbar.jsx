          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <div className="relative">
              <button
                ...
                onBlur={() =>
                  setTimeout(() => setCategoriasAbiertas(false), 150)
                }
              ...
              {wishlistAbierta && (
                <div className="absolute top-full right-0 mt-2 w-72 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl overflow-hidden">
                  <div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
                    <h3 className="text-zinc-800 dark:text-white font-bold text-sm">
                      Favoritos
                    </h3>
                    <button
                      onClick={() => setWishlistAbierta(false)}
                      className="text-zinc-400 hover:text-zinc-700 dark:hover:text-white transition text-xs"
                    >
                      <X size={14} />
                    </button>
                  </div>
                  {wishlist.length === 0 ? (
                    ...
                    <div>
                      {wishlist.map((p) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition group"
                        >
                          <Link
                            href={`/productos/${p.id}`}
                            onClick={() => setWishlistAbierta(false)}
                          ...
                          >
                            ...
                            />
                            <div className="min-w-0">
                              <p className="text-zinc-800 dark:text-white text-sm truncate">
                                {p.nombre}
                              </p>
                              <p className="text-green-400 text-xs font-bold">
                                ${precioFinal(p).toLocaleString()}
        ...
        {menuAbierto && (
          <div className="md:hidden bg-white dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800 px-6 py-4 flex flex-col gap-2">
            <Link
              ...
              className="text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white py-3 border-b border-zinc-200 dark:border-zinc-800 transition"
            >
              Inicio
            </Link>
            <Link
            ...
            <Link
              href="/carrito"
              onClick={() => setMenuAbierto(false)}
              className="text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white py-3 transition"
            >
              Carrito{" "}
              {totalItems > 0 && (
                <span className="bg-green-400 text-black text-xs font-black px-2 py-0.5 rounded-full ml-2">
                  {totalItems}
                </span>
              )}
            </Link>
