...
import { ArrowRight, Shield, Truck, Star, Keyboard, Mouse, Headphones, Mic, Zap, RotateCcw, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function Home() {
  ...
  const precioFinal = (p) => {
    ...
    return p.descuento > 0 ? p.precio - (p.precio * p.descuento) / 100 : p.precio
  }
...
<div className="absolute inset-0 bg-[linear-gradient(to_right,#00000008_1px,transparent_1px),linear-gradient(to_bottom,#00000008_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#ffffff04_1px,transparent_1px),linear-gradient(to_bottom,#ffffff04_1px,transparent_1px)] bg-[size:50px_50px]" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-8 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-24">
          ...
          <div ref={heroRef} className="flex flex-col gap-8">
            <div className={`opacity-0-init ${heroInView ? "animate-slide-in" : ""}`}>
              <span className="inline-flex items-center gap-2 text-green-400 text-xs font-mono tracking-widest uppercase border border-green-400/30 bg-green-400/5 px-4 py-2 rounded-full">
                <span className="w-2 h-2 bg-green-400 rounded-full" style={{ animation: "pulseGreen 2s infinite" }} />
                Accesorios Gamer Premium · Colombia
              </span>
            </div>

            <div className={`opacity-0-init ${heroInView ? "animate-slide-in delay-100" : ""}`}>
            ...
            <p className={`text-zinc-400 text-lg leading-relaxed max-w-md opacity-0-init ${heroInView ? "animate-slide-in delay-200" : ""}`}>
              Teclados mecánicos, mouse de precisión, micrófonos y audífonos gamer. Todo lo que necesitas para dominar.
            </p>

            <div className={`flex gap-4 flex-wrap opacity-0-init ${heroInView ? "animate-slide-in delay-300" : ""}`}>
              <Link
                href="/productos"
...
>
  Ofertas del día
            ...
            <div className={`grid grid-cols-3 gap-6 pt-4 opacity-0-init ${heroInView ? "animate-slide-in delay-400" : ""}`}>
              {[
                { num: `${productos.length}+`, label: "Productos" },
                { num: "24h", label: "Envío express" },
                { num: "100%", label: "Garantía" },
              ].map((s) => (
                <div key={s.label} className="border-l border-zinc-300 dark:border-zinc-800 pl-4">
                  <p className="text-2xl font-black text-green-400">{s.num}</p>
                  <p className="text-zinc-500 dark:text-zinc-500 text-xs mt-0.5">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Carousel */}
          {productoActual && (
            <div className={`relative opacity-0-init ${heroInView ? "animate-slide-right delay-200" : ""}`}>
              <div className="relative bg-zinc-900 border border-zinc-800 rounded-3xl p-6 overflow-hidden">
                <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_#22c55e08_0%,_transparent_60%)]" />

                {/* Badge descuento */}
                {productoActual.descuento > 0 && (
                  <span className="absolute top-4 left-4 z-10 bg-green-400 text-black text-xs font-black px-3 py-1 rounded-full">
                    -{productoActual.descuento}% OFF
                  </span>
                )}

                {/* Imagen */}
                <div className="relative overflow-hidden rounded-2xl bg-zinc-800 aspect-square">
                  <img
                ...
                <div className="mt-5 flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="text-green-400 font-mono text-xs tracking-widest uppercase">{productoActual.categoria}</span>
                      <h3 className="text-white font-black text-xl leading-tight mt-1">{productoActual.nombre}</h3>
                    </div>
                    <div className="text-right flex-shrink-0">
                      {productoActual.descuento > 0 && (
                        <p className="text-zinc-600 line-through text-sm">${Number(productoActual.precio).toLocaleString()}</p>
                      )}
                      <p className="text-green-400 font-black text-2xl">${precioFinal(productoActual).toLocaleString()}</p>
                    </div>
                  </div>
                  ...
                  <BotonCarrito producto={{ ...productoActual, precio: precioFinal(productoActual) }} />
                </div>

                {/* Navegación carousel */}
                <div className="flex items-center justify-between mt-4">
                  <button
                    onClick={() => setIndiceCarousel((prev) => (prev - 1 + productos.length) % productos.length)}
                    ...
                    {productos.map((_, i) => (
                      <button
                        ...
                        className={`rounded-full transition-all ${
                          i === indiceCarousel ? "w-6 h-2 bg-green-400" : "w-2 h-2 bg-zinc-700 hover:bg-zinc-500"
                        }`}
                      />
                    ))}
                  </div>
                  <button
                    onClick={() => setIndiceCarousel((prev) => (prev + 1) % productos.length)}
                    className="w-8 h-8 bg-zinc-800 hover:bg-zinc-700 rounded-lg flex items-center justify-center transition"
                  >
      ...
      <div className="bg-green-400 py-3 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap">
          {Array.from({ length: 8 }, (_, i) => (
            <span key={i} className="text-black font-black text-xs mx-8 tracking-widest uppercase flex items-center gap-3">
      ...
      <section ref={catRef} className="px-8 py-20 max-w-7xl mx-auto">
        <div className={`text-center mb-12 opacity-0-init ${catInView ? "animate-slide-in" : ""}`}>
          <p className="text-green-400 font-mono text-xs tracking-widest uppercase mb-2">Explora</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: <Keyboard size={28} />, nombre: "Teclados", desc: "Mecánicos y membrana", color: "from-blue-500/10" },
            { icon: <Mouse size={28} />, nombre: "Mouse", desc: "Alta precisión", color: "from-purple-500/10" },
            { icon: <Headphones size={28} />, nombre: "Audífonos", desc: "Sonido inmersivo", color: "from-green-500/10" },
            { icon: <Mic size={28} />, nombre: "Micrófonos", desc: "Claridad de audio", color: "from-orange-500/10" },
          ].map((cat, i) => (
            <Link
              key={cat.nombre}
              href="/productos"
            ...
            >
              <div className={`w-16 h-16 bg-gradient-to-br ${cat.color} to-transparent rounded-2xl flex items-center justify-center text-green-400 group-hover:scale-110 transition-transform duration-300 border border-zinc-200 dark:border-zinc-700`}>
                {cat.icon}
              </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex flex-col gap-3">
              ...
              <p className="text-zinc-400 max-w-sm">
                Aprovecha nuestras ofertas por tiempo limitado en los mejores accesorios gamer.
              </p>
            </div>
            <Link
              href="/productos"
      ...
      <section ref={productosRef} className="px-8 py-20 bg-zinc-50 dark:bg-zinc-900/30">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <div className={`opacity-0-init ${productosInView ? "animate-slide-left" : ""}`}>
              <p className="text-green-400 font-mono text-xs tracking-widest uppercase mb-2">Lo más popular</p>
              <h2 className="text-4xl font-black text-zinc-900 dark:text-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {productos.slice(0, 4).map((producto, i) => {
              ...
              return (
                <div
                  key={producto.id}
                  className={`bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden flex flex-col hover:border-green-400/60 hover:shadow-lg dark:hover:shadow-green-400/5 transition-all duration-300 hover:-translate-y-1 group opacity-0-init ${productosInView ? "animate-slide-in" : ""}`}
                  style={{ animationDelay: `${i * 0.1}s` }}
                >
      ...
      <section ref={statsRef} className="px-8 py-20 max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { num: "500+", label: "Clientes felices", icon: <Star size={24} /> },
            { num: "24h", label: "Envío express", icon: <Truck size={24} /> },
            { num: "100%", label: "Garantía real", icon: <Shield size={24} /> },
            { num: "30 días", label: "Devoluciones", icon: <RotateCcw size={24} /> },
          ].map((stat, i) => (
            <div
              key={stat.label}
      ...
      <section ref={bannerRef} className="px-8 py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto text-center flex flex-col items-center gap-6">
          <div className={`opacity-0-init ${bannerInView ? "animate-slide-in" : ""}`}>
            <p className="text-green-400 font-mono text-xs tracking-widest uppercase mb-4">Coam Tec</p>
          ...
          <p className={`text-zinc-500 text-lg max-w-md opacity-0-init ${bannerInView ? "animate-slide-in delay-100" : ""}`}>
            Envíos a todo Colombia. Garantía en todos los productos. Paga con tarjeta, PSE o contra entrega.
          </p>
          <div className={`flex gap-4 flex-wrap justify-center opacity-0-init ${bannerInView ? "animate-slide-in delay-200" : ""}`}>
            <Link
              href="/productos"
      ...
      <footer className="border-t border-zinc-200 dark:border-zinc-800 px-8 py-12 bg-zinc-50 dark:bg-zinc-900/50">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="md:col-span-2">
            ...
            <p className="text-zinc-500 text-sm mt-2 max-w-xs leading-relaxed">
              Los mejores accesorios gamer de Colombia. Teclados, mouse, audífonos y micrófonos para llevar tu setup al siguiente nivel.
            </p>
            <div className="flex gap-3 mt-4">
              {[
                ...
                <span key={item.label} className="flex items-center gap-1 text-xs text-zinc-500 bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 rounded-full border border-zinc-200 dark:border-zinc-700">
          ...
          <div>
            <h4 className="text-zinc-800 dark:text-white font-bold text-sm mb-4">Tienda</h4>
            <div className="flex flex-col gap-2">
              {["Productos", "Ofertas", "Categorías", "Novedades"].map((link) => (
                <Link key={link} href="/productos" className="text-zinc-500 hover:text-green-400 transition text-sm">
                  {link}
                </Link>
              ))}
            </div>
          ...
          <div>
            <h4 className="text-zinc-800 dark:text-white font-bold text-sm mb-4">Ayuda</h4>
            <div className="flex flex-col gap-2">
              {["Envíos", "Devoluciones", "Garantía", "Contacto"].map((link) => (
                <Link key={link} href="/" className="text-zinc-500 hover:text-green-400 transition text-sm">
                  {link}
                </Link>
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto mt-8 pt-8 border-t border-zinc-200 dark:border-zinc-800 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-zinc-400 text-sm">© 2025 Coam Tec. Todos los derechos reservados.</p>
          <Link href="/admin" className="text-zinc-600 hover:text-green-400 transition text-xs">
            Admin
          </Link>
        </div>
      </footer>
