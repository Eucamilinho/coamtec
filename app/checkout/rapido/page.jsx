...
import { ChevronRight, Lock, Truck, Shield, CreditCard, Building2, Wallet } from "lucide-react"

const PASOS = ["Contacto", "Envío", "Pago"]

export default function CheckoutRapido() {
  const { items } = useCompraRapida()
            ...
            <div key={p} className="flex items-center gap-2">
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition ${
                ...
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-black ${
                  i === paso ? "bg-black/20" : i < paso ? "bg-green-400 text-black" : "bg-zinc-300 dark:bg-zinc-700 text-zinc-500"
                }`}>
                  {i < paso ? "✓" : i + 1}
                </span>
                {p}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
              ...
              <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    <label className="text-zinc-500 text-sm">Nombre completo</label>
                    <input
                      name="nombre"
                  ...
                  <div className="flex flex-col gap-1 md:col-span-2">
                    <label className="text-zinc-500 text-sm">Teléfono / WhatsApp</label>
                    <input
                      name="telefono"
            ...
            {paso === 1 && (
              <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    <label className="text-zinc-500 text-sm">Departamento</label>
                    <input
                      name="departamento"
                  ...
                  <div className="flex flex-col gap-1 md:col-span-2">
                    <label className="text-zinc-500 text-sm">Referencia</label>
                    <textarea
                      name="referencia"
                ...
                <div className="flex flex-col gap-2 mt-2">
                  <label className="text-zinc-500 text-sm font-medium">Método de envío</label>
                  {[
                    { id: "estandar", label: "Envío estándar", desc: "3 a 5 días hábiles", precio: 15000 },
                    { id: "express", label: "Envío express", desc: "1 a 2 días hábiles", precio: 25000 },
                  ].map((opcion) => (
                    <label
                      key={opcion.id}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <input type="radio" name="envio" value={opcion.id} onChange={handleChange} className="accent-green-400" />
                        <div>
                          <p className="text-zinc-800 dark:text-white font-medium text-sm">{opcion.label}</p>
                          <p className="text-zinc-500 text-xs">{opcion.desc}</p>
                        </div>
                      </div>
                      <span className="text-green-400 font-bold text-sm">${opcion.precio.toLocaleString()}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            {paso === 2 && (
              <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  {[
                    { id: "tarjeta", icon: <CreditCard size={20} />, label: "Tarjeta débito / crédito", desc: "Visa, Mastercard, Amex" },
                    { id: "pse", icon: <Building2 size={20} />, label: "PSE", desc: "Débito desde tu banco" },
                    { id: "contraentrega", icon: <Wallet size={20} />, label: "Contra entrega", desc: "Paga cuando recibas" },
                  ].map((metodo) => (
                    <label
                      key={metodo.id}
                    ...
                    >
                      <input type="radio" name="metodoPago" value={metodo.id} onChange={handleChange} className="accent-green-400" />
                      <div className="text-green-400">{metodo.icon}</div>
                      <div>
                        <p className="text-zinc-800 dark:text-white font-medium text-sm">{metodo.label}</p>
                ...
                <div className="bg-green-400/5 border border-green-400/20 rounded-xl p-4 flex flex-col gap-2 mt-2">
                  <p className="text-zinc-700 dark:text-zinc-300 text-sm font-medium">Resumen</p>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const res = await fetch("/api/crear-preferencia", {
                        ...
                        body: JSON.stringify({ items, formulario, subtotal, envio, total }),
                      })
          ...
          <div>
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 sticky top-24 flex flex-col gap-4">
              <h3 className="font-bold text-zinc-800 dark:text-white">Tu pedido</h3>
              <div className="flex flex-col gap-3">
                {items.map((item, i) => (
                  <div key={i} className="flex gap-3 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-zinc-800 dark:text-white text-sm font-medium truncate">{item.nombre}</p>
                    </div>
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  { icon: <Shield size={12} />, label: "Garantía en todos los productos" },
                ].map((item) => (
